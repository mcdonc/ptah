...
class ApplicationsModuleView(view.View):
    view.pview(
        ...
        template=view.template('templates/module-apps.pt'))
...
class SharingForm(form.Form):
    view.pview(
        context = AppFactory,
        template = view.template('templates/module-apps-sharing.pt'))
...
class TypeIntrospection(object):
    ...
    actions = view.template('templates/directive-type.pt')
